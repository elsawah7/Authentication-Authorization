...
<form action="{{ url('login') }}" method="POST" class="space-y-4">
    ...
    <div>
        ...
        <input type="text" id="identifier" name="identifier" autocomplete="off" autofocus
    ...
    <div class="flex justify-between mt-4">
        @foreach (config('social.providers') as $provider)
            <a href="{{ url($provider['url']) }}"
                class="flex items-center justify-center w-1/3 py-3 bg-{{ $provider['color'] }}-500 rounded-lg font-semibold text-white hover:bg-{{ $provider['color'] }}-600 focus:outline-none focus:ring-2 focus:ring-{{ $provider['color'] }}-500 mr-2">
