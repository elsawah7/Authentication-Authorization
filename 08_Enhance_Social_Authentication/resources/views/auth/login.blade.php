...
<form action="{{ url('login') }}" method="POST" class="space-y-4">
    ...
    <div class="flex justify-between mt-4">
        @foreach (config('social.providers') as $provider)
            <a href="{{ $provider['url'] }}"
                class="flex items-center justify-center w-1/3 py-3 bg-{{ $provider['color'] }}-500 rounded-lg font-semibold text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 mr-2">
