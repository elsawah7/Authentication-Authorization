                ...
                <tbody>
                    @foreach ($users as $users => $user)
                        <tr class="hover:bg-gray-700">
                            <td class="p-3 border-b border-gray-700">{{ $users + 1 }}</td>
                            <td class="p-3 border-b border-gray-700">{{ $user->name }}</td>
                            <td class="p-3 border-b border-gray-700">{{ $user->email }}</td>
                            <td
                                class="p-3 border-b border-gray-700 text-{{ $user->account_verified_at ? 'green' : 'red' }}-700">
                                {{ $user->account_verified_at ? 'Yes' : 'No' }}</td>
                            <td class="p-3 border-b border-gray-700">{{ $user->roles->pluck('name')->implode(', ') }}
                            </td>
                            <td class="p-3 border-b border-gray-700">
                                @hasPermissionTo(\App\Enums\PermissionEnum::CHANGE_USER_ROLES->value)
                                    <button class="bg-blue-600 text-white px-4 py-2 rounded"
                                        onclick="openRoleModal({{ $user->id }}, {{ json_encode($user->roles->pluck('id')) }})">Change
                                        Role</button>
                                @endhasPermissionTo
                            </td>
                        </tr>
    ...
    <div id="roleModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center">
        <div class="bg-gray-800 p-6 rounded shadow-md w-96">
            ...
            <form id="roleForm" action="" method="get">
                <select name="role_ids[]" id="roleSelect" multiple
                ...
                <div class="flex justify-end mt-4">
                    <button type="button" class="bg-gray-600 text-white px-4 py-2 rounded mr-2"
